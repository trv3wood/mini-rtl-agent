...
from .skillrouter_export import prepare_skillrouter_benchmark_data, prepare_skillrouter_query_data
...
def build_skillrouter_rerank_command(
    *,
    ...
    python = external_python(external_root)
    script = Path(__file__).resolve().parents[2] / "scripts" / "skillrouter_rerank_query.py"
    return [
        str(python),
        str(script),
        "--external-root",
