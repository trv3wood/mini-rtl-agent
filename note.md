...
```
   ...
   - `evidence.json` is now the first deterministic EvidencePack layer:
     ports, parameters, instances, clock/reset candidates, memory candidates, always blocks, continuous assignments, assertions, comments, FSM hints, source locations, and source compile status are emitted with stable evidence IDs.
   ...
   - `quality.json` now includes explicit quality gates for metadata schema, dependency closure, duplicate definitions, source compile, generated TB compile, smoke simulation, original tests, formal verification, and manual review.
   - `report.json` aggregates `quality_gate_counts` across generated skills.
...
- Quality gate regression checks verify passed/failed/absent gate states and report-level `quality_gate_counts`.
...
- EvidencePack currently covers deterministic structural facts, including memory candidates, always blocks, continuous assignments, assertions, and staged verification facts. `skill_spec.json` now uses a dedicated evidence-aware LLM prompt, but the quality of semantic claims still depends on the configured provider/model and available structural evidence.
...
- Quality gates are explicit but still shallow: they separate metadata, closure, compile, smoke simulation, original tests, formal verification, and manual review states, but only the first five can currently pass automatically. Original tests, formal verification, and manual review remain `absent`.
...
2. Enrich EvidencePack extraction:
   ...
   - richer always-block summaries beyond sensitivity lists
   - richer assertion classification beyond observed assertion text
